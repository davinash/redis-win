...
void flushAppendOnlyFile(int force) {
    ...
    if (server.aof_no_fsync_on_rewrite &&
        (server.aof_child_pid != INVALID_HANDLE || server.rdb_child_pid != INVALID_HANDLE))
            return;
...
int rewriteListObject(rio *r, robj *key, robj *o) {
    ...
    if (o->encoding == REDIS_ENCODING_ZIPLIST) {
        ...
        while(ziplistGet(p,&vstr,&vlen,&vlong)) {
            if (count == 0) {
                int cmd_items = (items > REDIS_AOF_REWRITE_ITEMS_PER_CMD) ? REDIS_AOF_REWRITE_ITEMS_PER_CMD : (int)items;
...
int rewriteAppendOnlyFileBackground(void) {
    ...
    if (server.aof_child_pid != INVALID_HANDLE) return REDIS_ERR;
    start = ustime();
    if ((childpid = fork()) == 0) {
        char tmpfile[256];
...
void bgrewriteaofCommand(redisClient *c) {
    ...
    } else if (server.rdb_child_pid != INVALID_HANDLE) {
