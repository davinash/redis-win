...
#else
void zzlNext(unsigned char *zl, unsigned char **eptr, unsigned char **sptr) {
    unsigned char *local_eptr, *local_sptr;
    redisAssert(*eptr != NULL && *sptr != NULL);

    local_eptr = ziplistNext(zl,*sptr);
    if (local_eptr != NULL) {
        local_sptr = ziplistNext(zl,local_eptr);
        redisAssert(local_sptr != NULL);
    } else {
        /* No next entry. */
        local_sptr = NULL;
    }
...
void zzlPrev(unsigned char *zl, unsigned char **eptr, unsigned char **sptr) {
    ...
    local_sptr = ziplistPrev(zl,*eptr);
    if (local_sptr != NULL) {
        local_eptr = ziplistPrev(zl,local_sptr);
        redisAssert(local_eptr != NULL);
    } else {
        /* No previous entry. */
        local_eptr = NULL;
    }
